...
This repository contains Docker configuration for running [LightRAG](https://github.com/HKUDS/LightRAG), a graph-based Retrieval Augmented Generation system that allows you to query your documents using powerful language models.

## Features

- 🔧 Easy setup with Docker and Docker Compose
- 🤖 Support for both local LLMs (via [Ollama](https://ollama.ai/)) and cloud LLMs (via OpenAI)
...
1. Clone this repository:
   ```bash
   git clone https://github.com/HKUDS/LightRAG-Docker.git
   cd LightRAG-Docker
   ```
...
2. Run the setup script:
   ...
   ```
   The script will guide you through the setup process, including choosing between Ollama and OpenAI. It generates several files that are not version-controlled:
   - `start.sh` and `stop.sh` (convenience scripts)
   - `.env` (environment configuration)
   - `data/` directory (for storage)

3. Start LightRAG using the generated script:
   ```bash
   ./start.sh
   ```

4. Access the web interface at http://localhost:9621
   - Default login: admin / admin123

## Configuration

All configuration options are stored in the `.env` file (generated by the setup script). You can modify this file to change:

- Server settings (port, workers, etc.)
...
1. Place your documents in the `inputs` folder
2. They will be automatically processed and indexed
3. Supported formats include PDF, TXT, DOCX, and more

### Querying Documents

1. Log in to the web interface (http://localhost:9621)
2. Use the chat interface to ask questions about your documents
3. LightRAG will retrieve relevant information and generate answers

## Advanced Configuration

### Changing LLM Model

Edit the `.env` file and modify the `LLM_MODEL` variable:
...
Edit the `.env` file to adjust memory limits:
...
2. Set a strong `TOKEN_SECRET` value
...
- **Ollama Connectivity**: If using Ollama, ensure it's running and accessible
- **Resource Limits**: If you experience out-of-memory errors, adjust the memory limits in `.env`
...
- [LightRAG](https://github.com/HKUDS/LightRAG) - The original project by HKUDS
- [Ollama](https://ollama.ai/) - For local LLM processing
- [OpenAI](https://openai.com/) - For cloud LLM services
